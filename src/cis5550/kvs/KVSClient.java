...
public class KVSClient implements KVS {
  ...
  public void delete(String oldTableName) throws IOException {
    ...
    for (WorkerEntry w : workers) {
      try {
        HTTP.doRequest("PUT", "http://"+w.address+"/delete/"+java.net.URLEncoder.encode(oldTableName, "UTF-8")+"/", null).body();
      } catch (Exception e) {}
    }
  }

  public void deleteInMemRow(String tableName,String rowName) throws IOException {
    if (!haveWorkers)
      downloadWorkers();

    for (WorkerEntry w : workers) {
      try {
        HTTP.doRequest("PUT", "http://"+
                w.address+"/delete/"+java.net.URLEncoder.encode(tableName, "UTF-8")+
                "/"+java.net.URLEncoder.encode(rowName, "UTF-8")+"/", null).body();
      } catch (Exception e) {}
    }
  }
